...
using namespace std;
int N, M, Q, sq = 400;
int L[MAXN], A[MAXN], shift[MAXN], blksum[410], indx[MAXN],
    rightmost[MAXN][410];
...
long long query(int lm, int rm) {
    ...
    // cout<<LB<<" "<<RB<<"\n";
    long long ans = 0;
    for (int i = LB; i < RB; i++) ans += blksum[i];
    // return ans;
    for (int i = LB * sq; i < lm; i++) ans -= valueof(i);
    for (int i = RB * sq; i <= rm; i++) ans += valueof(i);
    return ans;
}
int main() {
